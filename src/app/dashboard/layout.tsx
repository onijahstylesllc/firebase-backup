...
export default function DashboardLayout({ children }: PropsWithChildren) {
  ...
  const { data: userProfile } = useSupabaseCollection<Profile>('profiles');

  const userName = userProfile?.[0]?.name || 'User';
  const avatarUrl = userProfile?.[0]?.profile_picture_url || '';

  return (
    <div className="grid min-h-screen w-full sm:grid-cols-1 md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      <div className="hidden border-r bg-muted/40 md:block">
        <div className="flex h-full max-h-screen flex-col gap-2">
          <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
            <Link href="/" className="flex items-center gap-2 font-semibold">
              <Bot className="h-6 w-6" />
              <span className="">AI Toolbox</span>
            </Link>
          </div>
      ...
      <div className="flex flex-col">
        <header className="flex h-14 items-center gap-4 border-b bg-muted/40 px-4 lg:h-[60px] lg:px-6">
          ...
          <div className="w-full flex-1">
            {/* Add nav links, breadcrumbs, etc. here */}
          </div>
          <ThemeToggle />
          <DropdownMenu>
